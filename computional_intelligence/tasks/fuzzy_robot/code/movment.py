...
class Robot:
    ...
    def avoid_distance(self, point_cloud, dt):
        ...
        desision, cf = model(
            variables=variables,
            rules=rules,
            s2=obs_distances["s2"] if obs_distances["s2"] is not None else 100,
            s3=obs_distances["s3"] if obs_distances["s3"] is not None else 100,
            s4=obs_distances["s4"] if obs_distances["s4"] is not None else 100,
        )
        desision = desision["decision"]
        # print(desision)
        if desision > 0 and self.count_down > 0:
            self.count_down -= dt
            self.move_left()
        # elif desision < 0 and self.count_down > 0:
        #     self.count_down -= dt
        #     self.move_right()
        else:
            self.count_down = 2
    ...
    def sensor_detect(self, point_cloud):
        ...
        # point[0] is radian , point[1] is [x, y]
        for point in points:
