...
class Robot:
    ...
    def avoid_distance(self, point_cloud, dt):
        ...
        if closest_obs is not None and closest_obs[1] < self.min_obs_dist and self.count_down > 0:
            ...
            s1 = obs_distances.get("s1", None)
            s2 = obs_distances.get("s2", None)
            s3 = obs_distances.get("s3", None)
            s4 = obs_distances.get("s4", None)
            s5 = obs_distances.get("s5", None)

        # fuzzy
            decision_output = model(
            variables={
               variables
            },
            rules=rules,
...
variables = {
    "s1": FuzzyVariable(
        universe_range=(30, 120),
        terms={
            "Far": [(100, 1), (80, 0.7), (60, 0.3), (60, 0)],
            "Close": [(60, 0), (50, 0.3), (40, 0.6), (30, 1)],
        },
    ),
    "s2": FuzzyVariable(
        universe_range=(30, 120),
